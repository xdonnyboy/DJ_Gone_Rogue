...
public class DJ_TileManagerScript : DJ_ManagerScript
{
	...
	public override void Start ()
	{
		...
		// change to access information from the LevelManager.
		//FromFile("");
		instantiateTiles();
	...
	*/
	...
	void instantiateTiles()
	{
		...
		Debug.Log("In TileManager accessing " + DJ_LevelParser.tileList.Count + " amount of tiles from DJ_LevelParser");
		foreach( Tile tile in DJ_LevelParser.tileList){
			GameObject _t = (GameObject.Instantiate(m_tilePrefab)) as GameObject;
			_t.transform.parent = transform;
			_t.transform.position = new Vector3(tile.x, 0, tile.z);
			DJ_Point newPoint = new DJ_Point((int)(tile.x + .5f), (int)(tile.z + .5f));
